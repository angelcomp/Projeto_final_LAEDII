...
int lista_retirar_fila_por_posicao(fila *q, int indice) {
	struct no_fila *aux, *ant_aux;
	int i;
	aux = q->inicio;

	if(q->inicio == NULL || indice < 0) {
		// Fila vazia, nada pode ser removido
	...
	for(i = 0; i < indice; i++) {
		...
		// Se o indice for maior do que o número de itens na fila
		if(aux == NULL) {
			// Erro
			return 0;
		}
	...
	// Caso a fila seja unitária
	if(q->inicio == q->fim) {
		q->inicio = NULL;
		q->fim = NULL;
	}

	// Fila não unitária, mas removendo do início
	else if(aux == q->inicio) {
		q->inicio = aux->prox;
	}

	// Fila não unitária, mas removendo do final
	else if(aux == q->fim) {
		q->fim = ant_aux;
		q->fim->prox = NULL;
	}

	// Remover do meio da Fila
	else {
